const app = {
  ...
  searchValue: (search) => {
    ...
    search.results.forEach((result) => {
      if (result.poster_path === null) {
        ...
        page.innerHTML += `     
        <div data-id="${result.id}" data-type="${localStorage.getItem(
          `type`
        )}" class="teste">
  ...
  getSelectedMovie: () => {
    let newUrl = `https://api.themoviedb.org/3/${localStorage.getItem(
      "type"
  ...
  getCredits: () => {
    let url = ` https://api.themoviedb.org/3/${localStorage.getItem(
      "type"
    )}/${localStorage.getItem(
      "id"
    )}/credits?api_key=9c21b916909f5afdb748670f51f947e4&language=en-US`;
  ...
  getImages: () => {
    let url = `https://api.themoviedb.org/3/${localStorage.getItem(
      ...
      "id"
    )}/videos?api_key=9c21b916909f5afdb748670f51f947e4&language=en-US
  ...
  getSimilarMovies: () => {
    let url = `https://api.themoviedb.org/3/${localStorage.getItem(
      ...
      "id"
    )}/similar?api_key=9c21b916909f5afdb748670f51f947e4&language=en-US&page=1`;
    let req = new Request(url, {
      method: "GET",
      mode: "cors",
    });
  ...
  imagesData: (images) => {
    if (images.results.length === 0) {
      ...
      images.results.forEach((video) => {
        document.querySelector(
          ".trailer"
        ).innerHTML = `<iframe width="560" height="315" src="https://www.youtube.com/embed/${video.key}?controls=0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
      });
    }
  },
  similar: (similar) => {
